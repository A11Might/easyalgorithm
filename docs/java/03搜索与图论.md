...
1. 初始化距离，dist[1] = 0，dist[i] = 正无穷，因为第一步只遍历到起点，所以只有起点的距离是确定的。
...
- 优先队列，不支持修改堆中任意元素，所以需要存入冗余数据，也就是每次更新都会将更新距离后的点加入堆中，更新的操作次数是 m，所以堆中元素最多会有 m 个。时间复杂度是 m * logm，一般稀疏图 m <= n<sup>2</sup>，所以 logm < logn<sup>2</sup> = 2 * logn，所以也是 O(m * logn) 级别的。

注：冗余的数据可以通过布尔数组 st 判断去掉：如果当前节点之前出队过 st[ver] = true，则跳过。

当一个节点第一次出队时它的距离就是最短距离，然后它会更新它的所有出边，之后它再从队列中弹出时，因为已经是最短距离了，所以它不会再更新其他节点的距离，可以直接跳过。中任意元素，所以需要存入冗余数据（冗余的数据可以通过布尔数组 st 判断去掉），每次更新都会将更新距离后的点加入堆中，更新的操作次数是 m，所以堆中元素最多会有 m 个。时间复杂度是 m * logm，一般稀疏图 m <= n<sup>2</sup>，所以 logm < logn<sup>2</sup> = 2 * logn，所以也是 O(m * logn) 级别的。
...
当无解时，也就是不能走到终点时，由于负数边权的存在，终点的距离可能被其他长度是正无穷的距离更新，所以判断无解的条件为 distance > 0x3f3f3f3f / 2。
...
另一个应用：传递闭包，floyd 算法可以在 O(n<sup>3</sup>) 的时间复杂度内，求出任意两个点是否可以直接或者间接到达。
...
因为如果当前 n1 中的节点 x 要匹配的 n2 中的节点 j 已经被匹配过了，也就是 if (find(match[j]))，我们会尝试让匹配 j 的节点 match[j] 换一个 n2 中的节点匹配，这时如果没有将 st[j] 标记上，那么在 match[j] 节点在进行 find 重新匹配的时候会再次考虑节点 j。
