...
- 优先队列：不支持修改堆中任意元素，所以需要存入冗余数据，也就是每次更新都会将更新距离后的点加入堆中，更新的操作次数是 m，所以堆中元素最多会有 m 个。
...
```java
int n; // 点的数量
int h[] = new int[N], e[] = new int[N], ne[] = new int[N], idx; // 稀疏图，使用邻接表存储所有边
...
```java
int n; // 总点数
int h[] = new int[N], w[] = new int[N], e[] = new int[N], ne[] = new int[N], idx; // 邻接表存储所有边
...
- 求负环时由于只需要判断负环是否存在，不需要求出路径具体长度的绝对值，因此 dist 数组就不需要初始化了。
...
2. 枚举每条边 a - b，权重为 w，如果 a，b 不连通，就将这条边加入集合中（并查集的应用：find(a) == find(b)、union(a, b)，时间复杂度为 O(m)）。
...
![3-6](03搜索与图论.assets/3-6.jpg)

注：布尔数组 st 的作用是对于 n1 中的任意一点，保证只考虑 n2 中每个点一次。

因为如果当前 n1 中的节点 x 要匹配的 n2 中的节点 j 已经被匹配过了，我们会尝试让匹配 j 的节点 match[j] 换一个 n2 中的节点匹配，也就是 if (find(match[j]))，这时如果没有将 st[j] 标记上，那么在 match[j] 节点在进行 find 重新匹配的时候会再次考虑节点 j。
