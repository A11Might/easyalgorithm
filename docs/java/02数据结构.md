...
单链表中的每个节点有两个属性：节点的值和指向下一个节点的指针。我们使用整型数组 e 存储节点的值，整型数组 ne 存储节点的 next 指针，并使用下标将 e 和 ne 关联起来。
...
双链表中的每个节点有三个属性：节点的值、指向上一个节点和指向下一个节点的指针。我们使用整型数组 e 存储节点的值，整型数组 l 存储节点的 pre 指针，整型数组 r 存储节点的 next 指针，并使用下标将 e 和 l, r 关联起来。
...
2. 循环队列
   ...
   队列中的元素个数：如果 tt > hh，个数就是 tt - hh；如果 hh == tt，个数是 0；如果 hh > tt，个数就是 tt + n - hh，n 是队列数组的长度。
...
优化 - 路径压缩：查到树根 root 后，将查找路径上的点 x 直接连到树根上 p[x] = root。
...
小根堆的递归定义：根节点的值小于等于左右两个子节点，所以树根节点就是堆中的最小值。

大根堆的递归定义：根节点的值大于等于左右两个子节点，所以树根节点就是堆中的最大值。
...
- 支持的操作（以小根堆为例）：
  ...
  - 删除任意一个元素：与上类似，不同的是如果 heap[k] 的值变大了，则需要向下堆化；如果 heap[k] 的值变小了，则需要向上堆化。由于只会出现一种情况，所以我们可以直接同时执行 down(k); up(k);，它们中只有一个会被执行。
...
- 下标从 1 开始，这样 x 的左儿子就是 2x, 右儿子就是 2x + 1。

- 插入元素时，我们应该先建立 ph 和 hp 的映射关系，再进行堆化，堆化过程中会不断维护这个映射关系。
...
int size;
int[] h = new int[N], ph = new int[N], hp = new int[N];
...
哈希函数：f(x) = (x % N + N) % N，其中 +N 再 %N 是为了保证 k 为正值，因为当 x 为负数时，%N 的结果也为负数。

其中取模的数 N 如果是质数，冲突的概率会低一些；如果是合数，比如是 2 的倍数，那么所有被 2 整除的数就不会被分到奇数位置上，分散的程度会变低，冲突概率就会变大。
...
字符串前缀哈希法（字符串下标从 1 开始）：h[k] 存储字符串前 k 个字母的哈希值。
